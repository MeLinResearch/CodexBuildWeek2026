...
def apply_and_verify_live_patch(store: Store, patch: PatchRow) -> dict[str, object]:
    run = store.get_run(patch.run_id)
    if run is None or run.mode != "live" or run.state != "PATCH_APPROVED" or patch.status != "approved":
        raise LiveRerunError("live patch is not approved for rerun")
    try:
    ...
    source_artifact = next((row for row in artifacts if row.kind == "input" and row.path.endswith(".csv")), None)
    ...
    if source is None or source.is_symlink() or not source.is_file():
        raise LiveRerunError("live source artifact is missing")
    try:
        source.resolve().relative_to(config.REPO_ROOT.resolve())
    except ValueError as error:
        raise LiveRerunError("live source artifact is outside the repository") from error
    ...
    with tempfile.TemporaryDirectory(prefix=f"release-assurance-{run.run_id}-") as temporary:
        ...
        verified = _run([sys.executable, "-I", str(verifier), str(target), str(source)], workspace,
                        env=verifier_env)
        try:
    ...
    report = {"run_id": run.run_id, "patch_id": patch.patch_id, "status": "passed",
              "pre_apply_tree": pre_tree, "post_apply_tree": post_tree, **result}
    ...
    store.insert_artifact(ArtifactRow(artifact_id, run.run_id, "test_output",
        ...
        "deterministic", "live", "none", "not_required", store.clock()))
